        ...
        {!cart || cart.items.length === 0 ? (
          ...
          <>
              ...
              {cart.items.map((item) => (
                <div className="drawer-item" key={item.id}>
                  ...
                  <div className="di-meta">
                    ...
                    <small>{item.variant_name}{item.is_preorder ? ' · Pre-order' : ''}</small>
                    {item.price_changed && (
                    ...
                    <div className="di-actions">
                      ...
                      <button className="textlink" onClick={() => remove(item.id)}>Remove</button>
                    </div>
                  </div>
            ...
            <div className="coupon-row">
              {cart.coupon_code ? (
                <>
                  ...
                  <button className="textlink" onClick={() => removeCoupon()}>Remove</button>
                </>
              ) : (
                <>
                  <input
            ...
            <div className="drawer-totals">
              <div><span>Subtotal</span><span>{inr(cart.totals.subtotal_paise)}</span></div>
              {cart.totals.discount_paise > 0 && (
                <div><span>Discount</span><span>−{inr(cart.totals.discount_paise)}</span></div>
              )}
              <div><span>GST included</span><span>{inr(cart.totals.tax_paise)}</span></div>
              <div className="grand"><span>Total</span><span>{inr(cart.totals.grand_total_paise)}</span></div>
