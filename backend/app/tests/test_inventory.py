...
def test_adjustment_ledger_and_negative_prevention(client, clean_db, product, staff_headers, manager):
    ...
    r = client.post("/api/v1/inventory/adjustments", headers=staff_headers,
                    json={"variant_id": m.id, "adjustment_type": "increase", "qty_change": 10,
...
def test_damage_and_defect_movements(client, clean_db, product, staff_headers):
    ...
    before = m.stock_qty
    r = client.post("/api/v1/inventory/adjustments", headers=staff_headers,
                    json={"variant_id": m.id, "adjustment_type": "damage", "qty_change": -2,
                          "reason": "water damage"})
    assert r.status_code == 201
    clean_db.expire_all()
...
def test_concurrent_reservation_no_oversell(client, clean_db, warehouse):
    ...
    def buy(i):
        ...
        with TestClient(app) as c:
            ...
            addr = c.post("/api/v1/addresses", headers=headers, json={
                "full_name": racers[i].full_name, "phone": "9876500000", "line1": "1 Race St",
                "city": "Bhopal", "state": "Madhya Pradesh",
                "postal_code": "462001"})
            addr_id = addr.json()["id"]
            c.post("/api/v1/carts/me/items", headers=headers, json={"variant_id": variant_id, "qty": 1})
            r = c.post("/api/v1/checkout/orders", headers={**headers, "Idempotency-Key": f"race-{i}"},
...
def test_reservation_release_on_cancel(client, clean_db, product, customer_headers, manager_headers, address):
    ...
    r = client.post("/api/v1/checkout/orders", headers={**customer_headers, "Idempotency-Key": "rel-1"},
...
def test_restock_alert_fires_once(client, clean_db, warehouse, customer, customer_headers):
    ...
    client.post("/api/v1/inventory/adjustments", headers=staff_h,
...
def test_expired_reservation_sweep(client, clean_db, product, customer_headers, address, admin_headers):
    ...
    r = client.post("/api/v1/checkout/orders", headers={**customer_headers, "Idempotency-Key": "sweep-1"},
