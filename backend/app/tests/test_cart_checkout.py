...
def test_price_change_surfaced_and_blocks_checkout(
    client, clean_db, product, customer_headers, manager_headers, address
):
    m = variant_of(product, "M")
    _add(client, customer_headers, m.id, 1)
    # manager changes price
    r = client.patch(
        f"/api/v1/products/variants/{m.id}",
    ...
    r = client.post(
        ...
        headers={**customer_headers, "Idempotency-Key": "pc-1"},
...
def test_coupon_validation_matrix(client, clean_db, product, customer_headers, manager_headers, address):
    ...
    # min order
    clean_db.add(
        Coupon(code="BIGONLY", discount_type="fixed", discount_value=10000, min_order_paise=99_000_00)
    )
    # usage limit reached
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "cp-1"},
        json={"shipping_address_id": address.id, "payment_method": "cod"},
    )
    assert r.status_code == 201
    order = r.json()["order"]
...
def test_pincode_blocked_rejects_checkout(
    client, clean_db, product, customer_headers, admin_headers, address
):
    r = client.post(
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "pin-1"},
        json={"shipping_address_id": address.id, "payment_method": "cod"},
    )
...
def test_insufficient_stock_blocks_checkout(client, clean_db, warehouse, customer_headers, address):
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "stock-1"},
        json={"shipping_address_id": address.id, "payment_method": "cod"},
    )
    assert r.status_code == 409
...
def test_duplicate_checkout_prevented_by_idempotency_key(
    client, clean_db, product, customer_headers, address
):
    m = variant_of(product, "M")
    _add(client, customer_headers, m.id, 1)
    ...
    r3 = client.post("/api/v1/checkout/orders", headers=h, json={**payload, "customer_notes": "changed"})
...
def test_cod_order_created_with_reservation(client, clean_db, product, customer_headers, address):
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "cod-1"},
        json={"shipping_address_id": address.id, "payment_method": "cod"},
    )
    assert r.status_code == 201
    order = r.json()["order"]
...
def test_prepaid_order_and_payment_session(client, clean_db, product, customer_headers, address):
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "pre-1"},
...
def test_order_items_are_snapshots(client, clean_db, product, customer_headers, manager_headers, address):
    ...
    r = client.post(
        "/api/v1/checkout/orders",
        headers={**customer_headers, "Idempotency-Key": "snap-1"},
        json={"shipping_address_id": address.id, "payment_method": "cod"},
    )
