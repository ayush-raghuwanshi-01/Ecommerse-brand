...
def test_cod_confirm_commits_stock(client, clean_db, product, customer_headers, staff_headers, address):
    ...
    r = client.post(
        f"/api/v1/orders/{order['id']}/status", headers=staff_headers, json={"status": "processing"}
    )
...
def test_status_transition_guard(client, clean_db, product, customer_headers, staff_headers, address):
    ...
    r = client.post(
        f"/api/v1/orders/{order['id']}/status", headers=staff_headers, json={"status": "shipped"}
    )  # skips processing/packed
    assert r.status_code == 422


def test_customer_cancel_before_packing_and_manager_approval(
    ...
    r = client.post(
        f"/api/v1/orders/{order['id']}/cancel-decision", headers=manager_headers, json={"approve": True}
    )
...
def test_customer_cannot_cancel_after_packing(
    client, clean_db, product, customer_headers, staff_headers, address
):
    order = _cod_order(client, clean_db, product, customer_headers, address, key="ord-5")
    for status in ("processing", "packed"):
        r = client.post(
            f"/api/v1/orders/{order['id']}/status", headers=staff_headers, json={"status": status}
        )
        assert r.status_code == 200
    r = client.post(
        f"/api/v1/orders/me/{order['id']}/cancel-request",
...
def test_staff_cannot_edit_after_packing_but_manager_can(
...
def test_order_edit_revalidates_and_audits(
    client, clean_db, product, customer_headers, staff_headers, address
):
...
def test_audit_log_records_manager_actions(client, clean_db, product, manager_headers, manager):
    r = client.post(
        "/api/v1/products", headers=manager_headers, json={"name": "Audit Coat", "base_price_paise": 150000}
    )
    assert r.status_code == 201
...
def test_list_totals_accurate_with_eager_loads(
    client, clean_db, product, customer_headers, staff_headers, address
):
